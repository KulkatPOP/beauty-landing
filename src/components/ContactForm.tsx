...
export function ContactForm() {
  ...
  return <form name="colaboraciones-beauty" onSubmit={submit} className="grid gap-6 rounded-[1.5rem] bg-white p-6 shadow-[0_25px_80px_rgba(56,36,40,.08)] md:p-9">
    ...
    {status === "error" && <p role="alert" className="text-sm text-red-800">No fue posible enviar el formulario. Escríbeme a <a className="underline" href="mailto:marcas@sofiarose.com">marcas@sofiarose.com</a>.</p>}
    <button disabled={status === "sending"} className="mt-2 w-fit rounded-full bg-[#382428] px-7 py-4 text-xs font-bold uppercase tracking-[.18em] text-white transition duration-300 hover:bg-[#b7596d] disabled:cursor-wait disabled:opacity-50">{status === "sending" ? "Enviando…" : "Enviar brief →"}</button>
  </form>;
}
